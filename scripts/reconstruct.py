...
def pattern_match(target_folder, calib_file, outfile):
    """
    ...
    cam_points = valid_coords[:, ::-1].astype(np.float32)
    proj_points = pattern_image[mask > 0].astype(np.float32)
    ...
    #     R @ point_cloud.T + T
    # ).T
    point_cloud = np.vstack([point_cloud, T.flat])  # get the coords origin position
    colors = white_image[valid_coords[:, 0], valid_coords[:, 1]]
...
def pattern_match_patched(target_folder, calib_file, outfile):
    """
    Match patterns and build dense point cloud using triangulation
    Args:
        target_folder (str): target folder for reconstruction
...
def main():
    ...
    pattern_match(target_folder, "calib_result1.yml", "capoo1.ply")
    target_folder = os.path.join(BASE_PATH, SECOND_SHOT_TARGET)
    pattern_match(target_folder, "calib_result2.yml", "capoo2.ply")
