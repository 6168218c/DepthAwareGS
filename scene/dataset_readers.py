...
class SceneInfo(NamedTuple):
    ...
    ply_path: str
    projector_intrinsics: np.ndarray = None
...
def readColmapCameras(
    cam_extrinsics, cam_intrinsics, images_folder, pcd=None, resolution=4, train_idx=None, white_background=False
):
    cam_infos = []
    model_zoe = None
    ...
    for idx, key in enumerate(sorted(cam_extrinsics, key=lambda x: cam_extrinsics[x].name)):
        ...
        depth_path = os.path.join(
            os.path.join(os.path.dirname(images_folder), "depths"), os.path.basename(extr.name).replace(".jpg", ".npy")
        )
        ...
        if pcd is not None and idx in train_idx:
            ...
            K = np.array(
                [[focal_length_x, 0, width // resolution / 2], [0, focal_length_y, height // resolution / 2], [0, 0, 1]]
            )
            cam_coord = np.matmul(
                K, np.matmul(R.transpose(), pcd.points.transpose()) + T.reshape(3, 1)
            ...
            valid_idx = np.where(
                np.logical_and.reduce(
                    (
                        ...
                        cam_coord[1] / cam_coord[2] <= height // resolution - 1,
                    )
                )
            )[0]
            ...
            depthmap[
                np.round(cam_coord[1]).astype(np.int32).clip(0, height // resolution - 1),
                np.round(cam_coord[0]).astype(np.int32).clip(0, width // resolution - 1),
            ] = pts_depths
            depth_weight[
                np.round(cam_coord[1]).astype(np.int32).clip(0, height // resolution - 1),
                np.round(cam_coord[0]).astype(np.int32).clip(0, width // resolution - 1),
            ] = 1 / pcd.errors[valid_idx] if pcd.errors is not None else 1
            ...
            source_depth = np.load(depth_path)
            target = depthmap.copy()

            target = ((target != 0) * 255).astype(np.uint8)
            depthmap, depthloss = optimize_depth(
                source=source_depth, target=depthmap, mask=depthmap > 0.0, depth_weight=depth_weight
            )
            ...
            source, refined = (
                depth_colorize_with_mask(source_depth[None, :, :], dmindmax=(0.0, 5.0)).squeeze(),
...
def optimize_depth(source, target, mask, depth_weight, prune_ratio=0.001):
    ...
    while abs(loss_ema - loss_prev) > 1e-5:
        ...
        if iteration % 1000 == 0:
            print(
                f"ITER={iteration:6d} loss={loss.item():8.4f}, params=[{scale.item():.4f},{shift.item():.4f}], lr={optimizer.param_groups[0]['lr']:8.4f}"
            )
            loss_prev = loss.item()
...
def refineColmapWithIndex(path, train_index):
    ...
    ptsidx, cnt = np.unique(
        np.concatenate(train_ptsidxlist), return_counts=True
    )  # for 2D points (extr.xys, extr.point3D_ids)

    valid_ptsidx = ptsidx[cnt >= min(2, len(train_index))][
        1:
    ]  # 2view -> 3view: more restrict condition (COLMAP uses 3-view observed feature points)

    for tidx, key in enumerate(sorted(cam_extrinsics, key=lambda x: cam_extrinsics[x].name)):
        cam_valid = np.isin(cam_extrinsics[key].point3D_ids, valid_ptsidx)
        cam_extrinsics[key] = cam_extrinsics[key]._replace(
            point3D_ids=cam_extrinsics[key].point3D_ids[cam_valid], xys=cam_extrinsics[key].xys[cam_valid]
        )

    ### valid 3D points (removing the points only detected in 1 camera)
    ptsidx, cnt = np.unique(
        np.concatenate(total_ptsidxlist), return_counts=True
    )  # for 3D points (xyz, rgb, err from points3D.bin)
...
def unprojFullDepth(caminfo, ply_path, single=False):
    ...
    for i in range(len(caminfo)):
        ...
        for j in range(len(caminfo)):
            if j != i:
                pts_cam_j = np.matmul(
                    K, np.matmul(caminfo[j].R.transpose(), pts_world) + caminfo[j].T.reshape(3, 1)
                )  # (3, -1)
                valid_idx_j = np.logical_and.reduce(
                    (
                        pts_cam_j[2] > 0,
                        pts_cam_j[0] / pts_cam_j[2] >= 0,
                        pts_cam_j[0] / pts_cam_j[2] <= W - 1,
                        pts_cam_j[1] / pts_cam_j[2] >= 0,
                        pts_cam_j[1] / pts_cam_j[2] <= H - 1,
                    )
                )
                ...
                color_diff = (
                    np.abs(np.array(caminfo[j].image)[pts_cam_j[1], pts_cam_j[0]] - pts_color[valid_idx_j]).mean(axis=1)
                    <= 1
                )
    ...
    randidx = np.random.choice(
        np.arange(len(positions)), size=min(len(positions) // len(caminfo) * 2, 100_000), replace=False
    )
...
def pick_idx_from_360(path, train_idx, kshot, center, num_trials=100_000):
    ...
    for _ in tqdm(range(num_trials), desc="Choosing best indice"):
        ...
        radians = np.arccos(
            (vectors * vectors[:, None, :]).sum(axis=-1).clip(-0.99999, 0.99999)
        )  # (kshot,3) * (kshot,1,3) --> (kshot,kshot,3) --(sum)--> (kshot,kshot)
...
def readColmapSceneInfo(path, images, eval, kshot=1000, seed=0, resolution=4, white_background=False):
    ...
    if os.path.exists(scene_center_path) and eval:
        ...
        train_idx = (
            sorted(np.random.choice(train_idx, size=min(kshot, len(train_idx)), replace=False))
            if eval
            else np.arange(len(train_idx)).tolist()
        )
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", use_depth=False):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            if use_depth and os.path.exists(
                os.path.join(path, frame["file_path"].replace("/train/", "/depths_train/") + ".npy")
            ):
                ...
                if os.path.exists(os.path.join(path, frame["file_path"].replace("/train/", "/masks_train/") + ".png")):
                    mask = (
                        imageio.v3.imread(
                            os.path.join(path, frame["file_path"].replace("/train/", "/masks_train/") + ".png")
                        )[:, :, 0]
                        / 255.0
                    )
                else:
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png", use_depth=False):
    ...
    train_cam_infos = readCamerasFromTransforms(
        path, "transforms_train.json", white_background, extension, use_depth=use_depth
    )
    print("Reading Test Transforms")
    test_cam_infos = readCamerasFromTransforms(
        path, "transforms_test.json", white_background, extension, use_depth=use_depth
    )
...
def readStructuredSceneInfo(path, images, eval, kshot=2, seed=0, resolution=4, white_background=False):
    class StructuredExtrinsics(NamedTuple):
        name: str
        camera_id: int
        R: np.ndarray[float]
        tvec: np.ndarray[float]

    class StructuredIntrinsic(NamedTuple):
        id: int
        K: np.ndarray
        width: int
        height: int
    ...
    ## load split_idx.json
    reading_dir = "images" if images == None else images
    view_count = len(glob(os.path.join(path, reading_dir, "*.png")))

    seed_everything(seed)

    cam_idxs = np.arange(view_count).tolist()

    ### refineColmapWithIndex() remove the cameras and features except the train set
    cam_extrinsics = []
    cam_intrinsics = []
    for idx in cam_idxs:
        calib_data = cv2.FileStorage(os.path.join(path, "stereos", f"{idx}.yml"), cv2.FILE_STORAGE_READ)
        R = calib_data.getNode("R").mat()
        T = calib_data.getNode("T").mat()
        K = calib_data.getNode("cam_K").mat()
        ...
        width = int(calib_data.getNode("width").real())
        height = int(calib_data.getNode("height").real())
        cam_extrinsics.append(StructuredExtrinsics(name=f"{idx}.png", camera_id=idx, R=R, tvec=T))
        cam_intrinsics.append(StructuredIntrinsic(id=idx, K=K, width=width, height=height))

    train_idx = cam_idxs

    ### making pcd with the features captured from train_cam
    ply_path = os.path.join(path, "sparse_recon", "0.ply")
    pcd = fetchPly(ply_path)

    cam_infos = readStructuredCameras(
        cam_extrinsics=cam_extrinsics,
        cam_intrinsics=cam_intrinsics,
        images_folder=os.path.join(path, reading_dir),
        pcd=pcd,
    ...
    nerf_normalization = getNerfppNorm(train_cam_infos[:-1])
...
def readStructuredCameras(
    ...
    for idx, (extr, intr) in enumerate(zip(cam_extrinsics, cam_intrinsics)):
        ...
        image = Image.open(image_path).resize((width // resolution, height // resolution))
        depth_path = os.path.join(
            os.path.join(os.path.dirname(images_folder), "depths"), os.path.basename(extr.name).replace(".png", ".npy")
        )
        ...
        depthmap, depth_weight = None, None
        depthloss = 1e8
        if pcd is not None and idx in train_idx and os.path.exists(depth_path):
            depthmap, depth_weight = (
                np.zeros((height // resolution, width // resolution)),
                np.zeros((height // resolution, width // resolution)),
            )
            # we do not use the provided projection matrix here
            # K = np.array(
            #     [[focal_length_x, 0, width // resolution / 2], [0, focal_length_y, height // resolution / 2], [0, 0, 1]]
            # )
            cam_coord = np.matmul(R.transpose(), pcd.points.transpose()) + T.reshape(3, 1)
            ...
            if resolution != 1:
                source_depth = cv2.resize(source_depth, (width // resolution, height // resolution))

            source, refined = (
                depth_colorize_with_mask(source_depth[None, :, :], dmindmax=(0.0, 80.0)).squeeze(),
                depth_colorize_with_mask(depthmap[None, :, :], dmindmax=(0.0, 80.0)).squeeze(),
            )
            cv2.imwrite(f"debug/{idx:03d}_source.png", (source[:, :, ::-1] * 255).astype(np.uint8))
            cv2.imwrite(f"debug/{idx:03d}_refined.png", (refined[:, :, ::-1] * 255).astype(np.uint8))
            cv2.imwrite(f"debug/{idx:03d}_target.png", target)

            depthmap = source_depth
            ##########################################################

            ########################patterns##########################
            # pattern_path = os.path.join(
            #     os.path.join(os.path.dirname(images_folder), "patterns"),
            #     os.path.basename(extr.name).replace(".png", ".npy"),
            # )
            # mask_path = pattern_path.replace(".npy", ".png")
            # patterns = np.load(pattern_path)
            # patterns_mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE) > 0

            # projector_info = cv2.FileStorage(
            #     os.path.join(os.path.dirname(images_folder), "patterns", "projector.yml"), cv2.FileStorage_READ
            # )
            # proj_width = int(projector_info.getNode("width").real())
            # proj_height = int(projector_info.getNode("height").real())
...
sceneLoadTypeCallbacks = {
    ...
    "Structured": readStructuredSceneInfo,
}
